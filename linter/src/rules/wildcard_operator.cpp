#include "rules/wildcard_operator.h"

#include <Surelog/Common/NodeId.h>
#include <Surelog/Design/FileContent.h>
#include <Surelog/ErrorReporting/ErrorContainer.h>
#include <Surelog/SourceCompile/SymbolTable.h>
#include <Surelog/SourceCompile/VObjectTypes.h>

#include <iostream>
#include <string>
#include <string_view>
#include <vector>

#include "fix/fix_it.h"
#include "fix/source_manager.h"
#include "main/lint_diagnostics.h"
#include "main/lint_rules.h"
#include "utils/location_utils.h"
#include "utils/name_utils.h"

namespace SL = SURELOG;

auto CheckWildcardOperatorsFixable(const SL::FileContent* fileContent,
                                   SL::ErrorContainer* errors,
                                   SL::SymbolTable* symbols,
                                   FixSourceManager& sm)
    -> std::vector<LintDiagnostic> {
  std::vector<LintDiagnostic> diags;

  if (fileContent == nullptr || errors == nullptr || symbols == nullptr) {
    return diags;
  }

  SL::NodeId const kRoot = fileContent->getRootNode();
  if (!kRoot) {
    return diags;
  }

  const std::string filepath = GetFixFilepath(fileContent);
  if (filepath.empty()) {
    return diags;
  }

  if (!sm.isLoaded(filepath)) {
    (void)sm.loadFile(fileContent->getFileId());
  }

  for (SL::NodeId const kWildEq :
       fileContent->sl_collect_all(kRoot, SL::VObjectType::paBinOp_WildEqual)) {
    std::string_view symName = "<unknown>";

    SL::NodeId const kExprNode = fileContent->Parent(kWildEq);
    if (kExprNode) {
      SL::NodeId const kLeftOperand = fileContent->Child(kExprNode);
      if (kLeftOperand) {
        symName = ExtractName(fileContent, kLeftOperand, "<unknown>");
      }
    }

    const unsigned line = fileContent->Line(kWildEq);
    const unsigned col = GetColumnSafe(fileContent, kWildEq);

    if (line == 0 || col == 0 || !sm.isLoaded(filepath)) {
      ReportError(fileContent, kWildEq, symName,
                  verihogg_lint::LINT_WILDCARD_EQUALITY_OPERATOR, errors,
                  symbols);
      ReportError(fileContent, kWildEq, symName,
                  verihogg_lint::LINT_WILDCARD_INEQUALITY_OPERATOR, errors,
                  symbols);
      continue;
    }

    const unsigned col_0 = col - 1;
    const std::string slice =
        sm.getSlice(filepath, LineCol{.line = line, .col = col_0}, col_0 + 3);

    if (slice.size() < 3) {
      ReportError(fileContent, kWildEq, symName,
                  verihogg_lint::LINT_WILDCARD_EQUALITY_OPERATOR, errors,
                  symbols);
      ReportError(fileContent, kWildEq, symName,
                  verihogg_lint::LINT_WILDCARD_INEQUALITY_OPERATOR, errors,
                  symbols);
      continue;
    }

    const bool isEquality = (slice.at(0) == '=');
    const auto errId = isEquality
                           ? verihogg_lint::LINT_WILDCARD_EQUALITY_OPERATOR
                           : verihogg_lint::LINT_WILDCARD_INEQUALITY_OPERATOR;
    const std::string replacement = isEquality ? "==?" : "!=?";
    const std::string desc =
        isEquality ? "replace '=?=' with '==?'" : "replace '!?=' with '!=?'";

    ReportError(fileContent, kWildEq, symName, errId, errors, symbols);

    const FixRange op_range{
        .begin = FixLocation{.filename = filepath, .line = line, .col = col},
        .end = FixLocation{.filename = filepath, .line = line, .col = col + 3},
    };

    LintDiagnostic d;
    d.filepath = filepath;
    d.line = line;
    d.col = col;
    d.message = std::string(slice.substr(0, 3)) + " -> " + replacement;

    try {
      d.fixes.push_back(FixIt::Replace(op_range, replacement, desc));
    } catch (const std::exception& e) {
      std::cerr << "autofix: cannot create fix at " << filepath << ":" << line
                << ":" << col << ": " << e.what() << "\n";
      diags.push_back(std::move(d));
      continue;
    }

    diags.push_back(std::move(d));
  }

  return diags;
}
