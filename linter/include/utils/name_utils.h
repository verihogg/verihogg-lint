...
auto ExtractName(const SL::FileContent* fC, SL::NodeId node,
                 const std::string_view& defaultName = "<unknown>")
    -> std::string_view;

// Извлечь параметр цикла
// Используется для определения порпуска параметра
auto FindForLoopVariableName(const SL::FileContent* fC, SL::NodeId forNode)
    -> std::string_view;

// Извлечь имя переменной из шаблона Variable_decl_assignment.
// Используется для поиска имен переменных в объявлениях.
auto ExtractVariableName(const SL::FileContent* fC, SL::NodeId parentNode)
    -> std::string_view;

// Извлечение имени параметра из шаблона Param_assignment.
// Используется для поиска имен параметров в декларациях.
auto ExtractParameterName(const SL::FileContent* fC, SL::NodeId parentNode)
    -> std::string_view;

auto FindDirectRhsLhsName(const SL::FileContent* fC, SL::NodeId concatNode)
    -> std::string_view;

void CollectNames(const SL::FileContent* fC, SL::NodeId root,
