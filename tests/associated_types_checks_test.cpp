...
auto BasePath() -> fs::path {
  return fs::current_path() / ".." / ".." / "tests" / "AssociatedTypesChecks";
}
...
TEST(IllegalNetDatatypeTest, NoError) {
  const fs::path tests_path{BasePath() / "IllegalNetDatatype" / "NoError"};

  testCheckWithNoErrorsExpected(tests_path, CheckIllegalNetDatatype);
}

TEST(IllegalNetDatatypeTest, RaiseError) {
  const fs::path tests_path{BasePath() / "IllegalNetDatatype" / "RaiseError"};
